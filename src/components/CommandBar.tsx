...
export default function CommandBar({ children }: { children: React.ReactNode }) {
  ...
  const actions = [
    ...
    },
    {
      ...
      name: "Playground",
      shortcut: ["g", "p"],
      keywords: "go-playground",
      section: "Ir Para",
      perform: () => navigate("/playground"),
    },
  ];
  ...
  return (
    <KBarResults
      ...
      onRender={({ item, active }) =>
        typeof item === "string" ? (
          <div className="p-[8px_16px] text-[10px] uppercase bg-[#ffffff0d]  ">{item}</div>
        ) : (
      ...
      {action.shortcut?.length ? (
        <div aria-hidden className="grid grid-flow-col gap-1">
          {action.shortcut.map((shortcut: any) => (
            <kbd key={shortcut} className="bg-[#ffffff1a] p-[4px_8px] text-[#8f9ba8] uppercase">
              {shortcut}
            </kbd>
          ))}
        </div>
